<template>
  ...
  <script setup>
    ...
    const TextOfBurrHoleAndPocketForBulb =
        (punctureSite.value === '前角') ? '正中から'+ operationSide.value + 'に3cm、Coronal sutureの前方1cmにBurr holeを開けるべく、皮膚切開を行った。'
      ...
      const TextOfLumbarPunctureAndPocketForBulb = '穿刺レベル' + punctureLevel.value + 'の傍正中にlinearに皮膚切開を行った。'
      ...
      const TextOfVentPuncture = (punctureSite.value === '前角') ? '穿刺針をnasion, 外耳孔に向けて挿入した。':'穿刺針をnasionに向けて挿入した。'
      +'穿刺針よりCSFの流出を確認した。穿刺針を抜き、同じTractに近位ドレナージチューブを挿入した。'
      + measurementPoint.value + 'を基準に近位ドレナージチューブを'+ punctureLength.value +'cm挿入して固定した。'

      const TextOfAbdominalSide = '臍'+ abdominalSide.value +'外側に5cmの横切開を行った。、皮下組織を切開した後。鈍的に脂肪組織を剥離し、腹直筋外膜を露出した。'
      ...
      const TextOfConnectionAndClosure = '遠位チューブ、圧調整バルブ、近位チューブを接合した。CSFの流出を確認した後、遠位チューブはダグラス窩に向かって30cm挿入した。'
