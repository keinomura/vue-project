<template>
  <div>
    ...
    <v-row v-if="operation === 'あり'">
      <v-col cols="6" v-if="operation === 'あり'">
    ...
    <v-radio-group v-model="dentalTreatmentHistory" label="痛みについての歯科治療歴" inline>
...
const preSurgeryMedication = ref('');
...
const medications = ref(['テグレトール（カルバマゼピン）', 'ビムパット（ラコサミド）', 'パキシル', 'ロキソニン', 'ガバペン', 'アレビアチン']);
const additionalNotes = ref('');

// methods
const generateText = () => {
  const text = `病変側: ${lesionSide.value}, 痛みの領域: V1 - ${painArea.value.includes('V1') ? 'Yes' : 'No'}, V2 - ${painArea.value.includes('V2') ? 'Yes' : 'No'}, V3 - ${painArea.value.includes('V3') ? 'Yes' : 'No'}, 今回初発か再発か: ${recurrence.value}, 痛みの発症時期: ${onsetYear.value}年 ${onsetMonth.value || '未入力'}月,  今回手術予定日: ${scheduledSurgeryDate.value}, 発症からの年月: 痛みについての歯科治療歴: ${dentalTreatmentHistory.value}, 手術直前内服量: ${preSurgeryMedication.value}`;
  this.$emit('text-generated', text);
};
